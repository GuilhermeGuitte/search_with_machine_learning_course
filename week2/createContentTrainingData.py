...
if __name__ == '__main__':
    ...
    with multiprocessing.Pool() as p:
        ...
        all_labels_count = tqdm(p.imap(_label_filename, files), total=len(files))
        with open(output_file, 'w') as output:
            labels_count = []

            print("Counting categories...")
            for label_list in all_labels_count:
                for (cat, name) in label_list:
                    labels_count.append(cat)

            labels_count = Counter(labels_count)
            labels_count = [filtered_label for filtered_label, count in labels_count.items() if int(count) >= int(min_products)]

            print(f"Minimum products associated: %i - Total number of labels found: %i" % (min_products, len(labels_count)))
            print(labels_count)

            print("Writing results to %s" % output_file)
            for label_list in all_labels:
                for (cat, name) in label_list:
                    output.write(f'__label__{cat} {name}\n') if cat in labels_count else None
